...
Calendar.prototype = {
    ...
    init: function () {
       ...
       // opts.width = el.width();
        ...
        el.on("click", ".calendar-cell", function (e) {
            ...
            var year = parseInt(cellDate.split("year")[0]);
            var month = parseInt(cellDate.split("year")[1].split("月")[0]) - 1;
            var date = parseInt(cellDate.split("year")[1].split("月")[1].split("日")[0]);
        ...
        el.on("click", ".year-item", function (e) {
            ...
            var yearNum = yearText.split("year")[0];
    ...
    _createToolbar: function () {
        ...
        if (showModeBtn) {
            ...
            if (mode == "month") {
                s += '<span  class="btn calendar-select-active">month</span>'
                s += '<span class="btn">year</span>'
            }
            else {
                s += '<span  class="btn">月</span>'
                s += '<span class="btn calendar-select-active">year</span>'
            }
            s += '</div>'
            s += '</div>'
        }
        ...
        for (var i = 0; i < 4; i++) {
            ...
            for (var l = 0; l < 3; l++) {
                ...
                if (month == renderMonth) {
                    s += '<td title="' + year + 'year' + (renderMonth + 1) + '月" class="calendar-cell calendar-thisMonth">';
                }
                else {
                    s += '<td title="' + year + 'year' + (renderMonth + 1) + '月" class="calendar-cell">';
                }
        ...
        for (var i = 0; i < 6; i++) {
            ...
            for (var l = 0; l < 7; l++) {
                ...
                if (renderDate.getMonth() < newDate.getMonth()) {
                    s += '<td title="' + year + 'year' + month + '月' + date + '日" class="calendar-cell calendar-last-month-cell">';
                }
                else if (renderDate.getMonth() > newDate.getMonth()) {
                    s += '<td title="' + year + 'year' + month + '月' + date + '日" class="calendar-cell calendar-next-month-cell">';
                }
                else if (date == nowNum) {
                    s += '<td title="' + year + 'year' + month + '月' + date + '日" class="calendar-cell calendar-today">';
                }else if(day==week[0] || day==week[1]){
                	   s += '<td title="' + year + 'year' + month + '月' + date + '日" class="calendar-cell calendar-weekend">';
                }
                else {
                    s += '<td title="' + year + 'year' + month + '月' + date + '日" class="calendar-cell">';
                }
		        s += '<div class="calendar-date">';
		                ...
		                if(ins[a].solarMonth==month){
		                	if(ins[a].solarDate==date){
		                		...
		                		if(ins[a].holidayDate=="holiday"){
		                			...
		                			s += '<div class="calendar-work">W</div><span class="calendar-work-bg"></span>';
		                		}
		                	}
		                }
	                }
